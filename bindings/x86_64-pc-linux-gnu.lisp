;; Generated by :claw at 2022-03-22T18:10:13.344016Z
...
(defparameter %assimp::+config-export-blob-name+ "EXPORT_BLOB_NAME")
...
(defparameter %assimp::+config-import-fbx-read-weights+ "IMPORT_FBX_READ_WEIGHTS")
...
(defparameter %assimp::+config-import-md3-load-shaders+ "IMPORT_MD3_LOAD_SHADERS")
...
(defparameter %assimp::+config-import-mdl-hl1-read-animations+ "IMPORT_MDL_HL1_READ_ANIMATIONS")

(defparameter %assimp::+config-import-mdl-hl1-read-animation-events+ "IMPORT_MDL_HL1_READ_ANIMATION_EVENTS")

(defparameter %assimp::+config-import-mdl-hl1-read-attachments+ "IMPORT_MDL_HL1_READ_ATTACHMENTS")

(defparameter %assimp::+config-import-mdl-hl1-read-blend-controllers+ "IMPORT_MDL_HL1_READ_BLEND_CONTROLLERS")

(defparameter %assimp::+config-import-mdl-hl1-read-bone-controllers+ "IMPORT_MDL_HL1_READ_BONE_CONTROLLERS")

(defparameter %assimp::+config-import-mdl-hl1-read-hitboxes+ "IMPORT_MDL_HL1_READ_HITBOXES")

(defparameter %assimp::+config-import-mdl-hl1-read-misc-global-info+ "IMPORT_MDL_HL1_READ_MISC_GLOBAL_INFO")

(defparameter %assimp::+config-import-mdl-hl1-read-sequence-transitions+ "IMPORT_MDL_HL1_READ_SEQUENCE_TRANSITIONS")
...
(defparameter %assimp::+config-import-schema-document-provider+ "IMPORT_SCHEMA_DOCUMENT_PROVIDER")
...
(defparameter %assimp::+matkey-anisotropy-factor+ "$mat.anisotropyFactor")

(defparameter %assimp::+matkey-base-color+ "$clr.base")

(defparameter %assimp::+matkey-base-color-texture+ 12)

(defparameter %assimp::+matkey-blend-func+ "$mat.blend")

(defparameter %assimp::+matkey-bumpscaling+ "$mat.bumpscaling")

(defparameter %assimp::+matkey-clearcoat-factor+ "$mat.clearcoat.factor")

(defparameter %assimp::+matkey-clearcoat-normal-texture+ 20)

(defparameter %assimp::+matkey-clearcoat-roughness-factor+ "$mat.clearcoat.roughnessFactor")

(defparameter %assimp::+matkey-clearcoat-roughness-texture+ 20)

(defparameter %assimp::+matkey-clearcoat-texture+ 20)
...
(defparameter %assimp::+matkey-emissive-intensity+ "$mat.emissiveIntensity")
...
(defparameter %assimp::+matkey-glossiness-factor+ "$mat.glossinessFactor")

(defparameter %assimp::+matkey-metallic-factor+ "$mat.metallicFactor")

(defparameter %assimp::+matkey-metallic-texture+ 15)
...
(defparameter %assimp::+matkey-roughness-factor+ "$mat.roughnessFactor")

(defparameter %assimp::+matkey-roughness-texture+ 16)
...
(defparameter %assimp::+matkey-sheen-color-factor+ "$clr.sheen.factor")

(defparameter %assimp::+matkey-sheen-color-texture+ 19)

(defparameter %assimp::+matkey-sheen-roughness-factor+ "$mat.sheen.roughnessFactor")

(defparameter %assimp::+matkey-sheen-roughness-texture+ 19)

(defparameter %assimp::+matkey-shininess+ "$mat.shininess")

(defparameter %assimp::+matkey-shininess-strength+ "$mat.shinpercent")

(defparameter %assimp::+matkey-specular-factor+ "$mat.specularFactor")

(defparameter %assimp::+matkey-transmission-factor+ "$mat.transmission.factor")

(defparameter %assimp::+matkey-transmission-texture+ 21)

(defparameter %assimp::+matkey-transparencyfactor+ "$mat.transparencyfactor")

(defparameter %assimp::+matkey-twosided+ "$mat.twosided")

(defparameter %assimp::+matkey-use-ao-map+ "$mat.useAOMap")

(defparameter %assimp::+matkey-use-color-map+ "$mat.useColorMap")

(defparameter %assimp::+matkey-use-emissive-map+ "$mat.useEmissiveMap")

(defparameter %assimp::+matkey-use-metallic-map+ "$mat.useMetallicMap")

(defparameter %assimp::+matkey-use-roughness-map+ "$mat.useRoughnessMap")

(defparameter %assimp::+matkey-volume-attenuation-color+ "$mat.volume.attenuationColor")

(defparameter %assimp::+matkey-volume-attenuation-distance+ "$mat.volume.attenuationDistance")

(defparameter %assimp::+matkey-volume-thickness-factor+ "$mat.volume.thicknessFactor")

(defparameter %assimp::+matkey-volume-thickness-texture+ 21)
...
(defparameter %assimp::+texture-type-max+ 21)
...
(defparameter %assimp::+assimp-ai-real-text-precision+ 9)
...
(cffi:defcenum (%assimp::anim-behaviour :unsigned-int)
               "/home/borodust/devel/repo/aw-projects/aw-assimp/src/lib/assimp/include/assimp/anim.h:228:6"
               (:default 0)
               (:constant 1)
               (:linear 2)
               (:repeat 3)
...
(cffi:defcenum (%assimp::blend-mode :unsigned-int)
               "/home/borodust/devel/repo/aw-projects/aw-assimp/src/lib/assimp/include/assimp/material.h:482:6"
               (:default 0)
               (:additive 1)
               (:force32bit 2147483647))
...
(cffi:defbitfield (%assimp::importer-flags :unsigned-int)
                  "/home/borodust/devel/repo/aw-projects/aw-assimp/src/lib/assimp/include/assimp/importerdesc.h:61:6"
...
(cffi:defcenum (%assimp::light-source-type :unsigned-int)
               "/home/borodust/devel/repo/aw-projects/aw-assimp/src/lib/assimp/include/assimp/light.h:63:6"
               (:undefined 0)
               (:directional 1)
               (:point 2)
               (:spot 3)
...
(cffi:defcenum (%assimp::metadata-type :unsigned-int)
               "/home/borodust/devel/repo/aw-projects/aw-assimp/src/lib/assimp/include/assimp/metadata.h:64:14"
               (:bool 0)
               (:int32 1)
               (:uint64 2)
               (:float 3)
...
(cffi:defcenum (%assimp::morphing-method :unsigned-int)
               "/home/borodust/devel/repo/aw-projects/aw-assimp/src/lib/assimp/include/assimp/mesh.h:563:6"
               (:vertex-blend 1)
               (:morph-normalized 2)
               (:morph-relative 3)
               (:force32bit 2147483647))
...
(cffi:defcenum (%assimp::origin :unsigned-int)
               "/home/borodust/devel/repo/aw-projects/aw-assimp/src/lib/assimp/include/assimp/types.h:429:6"
               (:origin-set 0)
               (:origin-cur 1)
               (:origin-end 2)
               (:%ai-origin-enforce-enum-size 2147483647))

(cffi:defbitfield (%assimp::post-process-steps :unsigned-int)
                  "/home/borodust/devel/repo/aw-projects/aw-assimp/src/lib/assimp/include/assimp/postprocess.h:72:6"
...
(cffi:defbitfield (%assimp::primitive-type :unsigned-int)
                  "/home/borodust/devel/repo/aw-projects/aw-assimp/src/lib/assimp/include/assimp/mesh.h:378:6"
                  (:point 1)
                  (:line 2)
                  (:triangle 4)
                  (:polygon 8)
                  (:ngon-encoding-flag 16)
                  (:force32bit 2147483647))

(cffi:defcenum (%assimp::property-type-info :unsigned-int)
               "/home/borodust/devel/repo/aw-projects/aw-assimp/src/lib/assimp/include/assimp/material.h:558:6"
               (:float 1)
               (:double 2)
               (:string 3)
               (:integer 4)
...
(cffi:defcenum (%assimp::return :int)
               "/home/borodust/devel/repo/aw-projects/aw-assimp/src/lib/assimp/include/assimp/types.h:400:14"
               (:return-success 0)
               (:return-failure -1)
...
(cffi:defcenum (%assimp::shading-mode :unsigned-int)
               "/home/borodust/devel/repo/aw-projects/aw-assimp/src/lib/assimp/include/assimp/material.h:355:6"
               (:flat 1)
               (:gouraud 2)
               (:phong 3)
               (:blinn 4)
...
(cffi:defbitfield (%assimp::texture-flags :unsigned-int)
                  "/home/borodust/devel/repo/aw-projects/aw-assimp/src/lib/assimp/include/assimp/material.h:438:6"
                  (:invert 1)
                  (:use-alpha 2)
                  (:ignore-alpha 4)
                  (:force32bit 2147483647))
...
(cffi:defcenum (%assimp::texture-mapping :unsigned-int)
               "/home/borodust/devel/repo/aw-projects/aw-assimp/src/lib/assimp/include/assimp/material.h:144:6"
               (:uv 0)
               (:sphere 1)
               (:cylinder 2)
               (:box 3)
...
(cffi:defcenum (%assimp::texture-op :unsigned-int)
               "/home/borodust/devel/repo/aw-projects/aw-assimp/src/lib/assimp/include/assimp/material.h:81:6"
               (:multiply 0)
               (:add 1)
               (:subtract 2)
               (:divide 3)
...
(cffi:defcenum (%assimp::texture-type :unsigned-int)
               "/home/borodust/devel/repo/aw-projects/aw-assimp/src/lib/assimp/include/assimp/material.h:188:6"
               (:none 0)
               (:diffuse 1)
               (:specular 2)
               (:ambient 3)
...
(defparameter %assimp::+fp-nan+ 0)

(defparameter %assimp::+fp-infinite+ 1)

(defparameter %assimp::+fp-zero+ 2)

(defparameter %assimp::+fp-subnormal+ 3)

(defparameter %assimp::+fp-normal+ 4)
...
(cffi:defcstruct (%assimp::vertex-weight :size 8)
                 ...
                 (%assimp::weight %assimp::%real :offset 4))

(cffi:defcstruct (%assimp::bone :size 1120)
                 (%assimp::name (:struct %assimp::string) :offset 0)
                 (%assimp::num-weights :unsigned-int :offset 1028)
                 (%assimp::armature
                  (:pointer (:struct %assimp::node)) :offset 1032)
                 (%assimp::node (:pointer (:struct %assimp::node))
                  :offset 1040)
                 (%assimp::weights
...
(cffi:defcstruct (%assimp::mesh :size 1320)
                 ...
                 (%assimp::texture-coords-names
                  (:pointer (:pointer (:struct %assimp::string)))
                  :offset 1312))
...
(cffi:defcstruct (%assimp::camera :size 1084)
                 (%assimp::name (:struct %assimp::string) :offset 0)
                 (%assimp::position (:struct %assimp::vector3d)
                  :offset 1028)
                 (%assimp::up (:struct %assimp::vector3d) :offset
                 ...
                 (%assimp::orthographic-width :float :offset 1080))

(cffi:defcstruct (%assimp::scene :size 1160)
                 ...
                 (%assimp::name (:struct %assimp::string) :offset
                  120)
                 (%assimp::private :string :offset 1152))
...
(declaim (inline %assimp::get-material-float))

(cffi:defcfun ("__claw_aiGetMaterialFloat"
               %assimp::get-material-float)
              %assimp::return
              (%assimp::p-mat (:pointer (:struct %assimp::material)))
              (%assimp::p-key :string)
              (%assimp::type :unsigned-int)
...
(declaim (inline %assimp::get-material-integer))

(cffi:defcfun ("__claw_aiGetMaterialInteger"
               %assimp::get-material-integer)
              %assimp::return
              (%assimp::p-mat (:pointer (:struct %assimp::material)))
              (%assimp::p-key :string)
              (%assimp::type :unsigned-int)
...
(declaim (inline %assimp::matrix3are-equal))

(cffi:defcfun ("aiMatrix3AreEqual" %assimp::matrix3are-equal)
              :int
              (%assimp::a (:pointer (:struct %assimp::matrix3x3)))
              (%assimp::b (:pointer (:struct %assimp::matrix3x3))))

(declaim (inline %assimp::matrix3are-equal-epsilon))

(cffi:defcfun ("aiMatrix3AreEqualEpsilon"
               %assimp::matrix3are-equal-epsilon)
              :int
              (%assimp::a (:pointer (:struct %assimp::matrix3x3)))
              (%assimp::b (:pointer (:struct %assimp::matrix3x3)))
              (%assimp::epsilon :float))

(declaim (inline %assimp::matrix3determinant))

(cffi:defcfun ("aiMatrix3Determinant" %assimp::matrix3determinant)
              :float
              (%assimp::mat (:pointer (:struct %assimp::matrix3x3))))

(declaim (inline %assimp::matrix3from-matrix4))

(cffi:defcfun ("aiMatrix3FromMatrix4" %assimp::matrix3from-matrix4)
              :void
              (%assimp::dst (:pointer (:struct %assimp::matrix3x3)))
              (%assimp::mat (:pointer (:struct %assimp::matrix4x4))))

(declaim (inline %assimp::matrix3from-quaternion))

(cffi:defcfun ("aiMatrix3FromQuaternion"
               %assimp::matrix3from-quaternion)
              :void
              (%assimp::mat (:pointer (:struct %assimp::matrix3x3)))
              (%assimp::q (:pointer (:struct %assimp::quaternion))))

(declaim (inline %assimp::matrix3from-rotation-around-axis))

(cffi:defcfun ("aiMatrix3FromRotationAroundAxis"
               %assimp::matrix3from-rotation-around-axis)
              :void
              (%assimp::mat (:pointer (:struct %assimp::matrix3x3)))
              (%assimp::axis (:pointer (:struct %assimp::vector3d)))
              (%assimp::angle :float))

(declaim (inline %assimp::matrix3from-to))

(cffi:defcfun ("aiMatrix3FromTo" %assimp::matrix3from-to)
              :void
              (%assimp::mat (:pointer (:struct %assimp::matrix3x3)))
              (%assimp::from (:pointer (:struct %assimp::vector3d)))
              (%assimp::to (:pointer (:struct %assimp::vector3d))))

(declaim (inline %assimp::matrix3inverse))

(cffi:defcfun ("aiMatrix3Inverse" %assimp::matrix3inverse)
              :void
              (%assimp::mat (:pointer (:struct %assimp::matrix3x3))))

(declaim (inline %assimp::matrix3rotation-z))

(cffi:defcfun ("aiMatrix3RotationZ" %assimp::matrix3rotation-z)
              :void
              (%assimp::mat (:pointer (:struct %assimp::matrix3x3)))
              (%assimp::angle :float))

(declaim (inline %assimp::matrix3translation))

(cffi:defcfun ("aiMatrix3Translation" %assimp::matrix3translation)
              :void
              (%assimp::mat (:pointer (:struct %assimp::matrix3x3)))
              (%assimp::translation
               (:pointer (:struct %assimp::vector2d))))

(declaim (inline %assimp::matrix4add))

(cffi:defcfun ("aiMatrix4Add" %assimp::matrix4add)
              :void
              (%assimp::dst (:pointer (:struct %assimp::matrix4x4)))
              (%assimp::src (:pointer (:struct %assimp::matrix4x4))))

(declaim (inline %assimp::matrix4are-equal))

(cffi:defcfun ("aiMatrix4AreEqual" %assimp::matrix4are-equal)
              :int
              (%assimp::a (:pointer (:struct %assimp::matrix4x4)))
              (%assimp::b (:pointer (:struct %assimp::matrix4x4))))

(declaim (inline %assimp::matrix4are-equal-epsilon))

(cffi:defcfun ("aiMatrix4AreEqualEpsilon"
               %assimp::matrix4are-equal-epsilon)
              :int
              (%assimp::a (:pointer (:struct %assimp::matrix4x4)))
              (%assimp::b (:pointer (:struct %assimp::matrix4x4)))
              (%assimp::epsilon :float))
...
(declaim (inline
          %assimp::matrix4decompose-into-scaling-axis-angle-position))

(cffi:defcfun ("aiMatrix4DecomposeIntoScalingAxisAnglePosition"
               %assimp::matrix4decompose-into-scaling-axis-angle-position)
              :void
              (%assimp::mat (:pointer (:struct %assimp::matrix4x4)))
              (%assimp::scaling
               (:pointer (:struct %assimp::vector3d)))
              (%assimp::axis (:pointer (:struct %assimp::vector3d)))
              (%assimp::angle (:pointer %assimp::%real))
              (%assimp::position
               (:pointer (:struct %assimp::vector3d))))

(declaim (inline
          %assimp::matrix4decompose-into-scaling-euler-angles-position))

(cffi:defcfun ("aiMatrix4DecomposeIntoScalingEulerAnglesPosition"
               %assimp::matrix4decompose-into-scaling-euler-angles-position)
              :void
              (%assimp::mat (:pointer (:struct %assimp::matrix4x4)))
              (%assimp::scaling
...
(declaim (inline %assimp::matrix4decompose-no-scaling))

(cffi:defcfun ("aiMatrix4DecomposeNoScaling"
               %assimp::matrix4decompose-no-scaling)
              :void
              (%assimp::mat (:pointer (:struct %assimp::matrix4x4)))
              (%assimp::rotation
               (:pointer (:struct %assimp::quaternion)))
...
(declaim (inline %assimp::matrix4determinant))

(cffi:defcfun ("aiMatrix4Determinant" %assimp::matrix4determinant)
              :float
              (%assimp::mat (:pointer (:struct %assimp::matrix4x4))))

(declaim (inline %assimp::matrix4from-euler-angles))

(cffi:defcfun ("aiMatrix4FromEulerAngles"
               %assimp::matrix4from-euler-angles)
              :void
              (%assimp::mat (:pointer (:struct %assimp::matrix4x4)))
              (%assimp::x :float)
              (%assimp::y :float)
...
(declaim (inline %assimp::matrix4from-matrix3))

(cffi:defcfun ("aiMatrix4FromMatrix3" %assimp::matrix4from-matrix3)
              :void
              (%assimp::dst (:pointer (:struct %assimp::matrix4x4)))
              (%assimp::mat (:pointer (:struct %assimp::matrix3x3))))

(declaim (inline %assimp::matrix4from-rotation-around-axis))

(cffi:defcfun ("aiMatrix4FromRotationAroundAxis"
               %assimp::matrix4from-rotation-around-axis)
              :void
              (%assimp::mat (:pointer (:struct %assimp::matrix4x4)))
              (%assimp::axis (:pointer (:struct %assimp::vector3d)))
              (%assimp::angle :float))

(declaim (inline %assimp::matrix4from-scaling-quaternion-position))

(cffi:defcfun ("aiMatrix4FromScalingQuaternionPosition"
               %assimp::matrix4from-scaling-quaternion-position)
              :void
              (%assimp::mat (:pointer (:struct %assimp::matrix4x4)))
              (%assimp::scaling
               (:pointer (:struct %assimp::vector3d)))
...
(declaim (inline %assimp::matrix4from-to))

(cffi:defcfun ("aiMatrix4FromTo" %assimp::matrix4from-to)
              :void
              (%assimp::mat (:pointer (:struct %assimp::matrix4x4)))
              (%assimp::from (:pointer (:struct %assimp::vector3d)))
              (%assimp::to (:pointer (:struct %assimp::vector3d))))

(declaim (inline %assimp::matrix4inverse))

(cffi:defcfun ("aiMatrix4Inverse" %assimp::matrix4inverse)
              :void
              (%assimp::mat (:pointer (:struct %assimp::matrix4x4))))

(declaim (inline %assimp::matrix4is-identity))

(cffi:defcfun ("aiMatrix4IsIdentity" %assimp::matrix4is-identity)
              :int
              (%assimp::mat (:pointer (:struct %assimp::matrix4x4))))

(declaim (inline %assimp::matrix4rotation-x))

(cffi:defcfun ("aiMatrix4RotationX" %assimp::matrix4rotation-x)
              :void
              (%assimp::mat (:pointer (:struct %assimp::matrix4x4)))
              (%assimp::angle :float))

(declaim (inline %assimp::matrix4rotation-y))

(cffi:defcfun ("aiMatrix4RotationY" %assimp::matrix4rotation-y)
              :void
              (%assimp::mat (:pointer (:struct %assimp::matrix4x4)))
              (%assimp::angle :float))

(declaim (inline %assimp::matrix4rotation-z))

(cffi:defcfun ("aiMatrix4RotationZ" %assimp::matrix4rotation-z)
              :void
              (%assimp::mat (:pointer (:struct %assimp::matrix4x4)))
              (%assimp::angle :float))

(declaim (inline %assimp::matrix4scaling))

(cffi:defcfun ("aiMatrix4Scaling" %assimp::matrix4scaling)
              :void
              (%assimp::mat (:pointer (:struct %assimp::matrix4x4)))
              (%assimp::scaling
               (:pointer (:struct %assimp::vector3d))))

(declaim (inline %assimp::matrix4translation))

(cffi:defcfun ("aiMatrix4Translation" %assimp::matrix4translation)
              :void
              (%assimp::mat (:pointer (:struct %assimp::matrix4x4)))
              (%assimp::translation
               (:pointer (:struct %assimp::vector3d))))
...
(declaim (inline %assimp::quaternion-are-equal))

(cffi:defcfun ("aiQuaternionAreEqual" %assimp::quaternion-are-equal)
              :int
              (%assimp::a (:pointer (:struct %assimp::quaternion)))
              (%assimp::b (:pointer (:struct %assimp::quaternion))))

(declaim (inline %assimp::quaternion-are-equal-epsilon))

(cffi:defcfun ("aiQuaternionAreEqualEpsilon"
               %assimp::quaternion-are-equal-epsilon)
              :int
              (%assimp::a (:pointer (:struct %assimp::quaternion)))
              (%assimp::b (:pointer (:struct %assimp::quaternion)))
              (%assimp::epsilon :float))

(declaim (inline %assimp::quaternion-conjugate))

(cffi:defcfun ("aiQuaternionConjugate" %assimp::quaternion-conjugate)
              :void
              (%assimp::q (:pointer (:struct %assimp::quaternion))))

(declaim (inline %assimp::quaternion-from-axis-angle))

(cffi:defcfun ("aiQuaternionFromAxisAngle"
               %assimp::quaternion-from-axis-angle)
              :void
              (%assimp::q (:pointer (:struct %assimp::quaternion)))
              (%assimp::axis (:pointer (:struct %assimp::vector3d)))
              (%assimp::angle :float))

(declaim (inline %assimp::quaternion-from-euler-angles))

(cffi:defcfun ("aiQuaternionFromEulerAngles"
               %assimp::quaternion-from-euler-angles)
              :void
              (%assimp::q (:pointer (:struct %assimp::quaternion)))
              (%assimp::x :float)
              (%assimp::y :float)
...
(declaim (inline %assimp::quaternion-from-normalized-quaternion))

(cffi:defcfun ("aiQuaternionFromNormalizedQuaternion"
               %assimp::quaternion-from-normalized-quaternion)
              :void
              (%assimp::q (:pointer (:struct %assimp::quaternion)))
              (%assimp::normalized
               (:pointer (:struct %assimp::vector3d))))

(declaim (inline %assimp::quaternion-interpolate))

(cffi:defcfun ("aiQuaternionInterpolate"
               %assimp::quaternion-interpolate)
              :void
              (%assimp::dst (:pointer (:struct %assimp::quaternion)))
              (%assimp::start
               (:pointer (:struct %assimp::quaternion)))
              (%assimp::end (:pointer (:struct %assimp::quaternion)))
              (%assimp::factor :float))

(declaim (inline %assimp::quaternion-multiply))

(cffi:defcfun ("aiQuaternionMultiply" %assimp::quaternion-multiply)
              :void
              (%assimp::dst (:pointer (:struct %assimp::quaternion)))
              (%assimp::q (:pointer (:struct %assimp::quaternion))))

(declaim (inline %assimp::quaternion-normalize))

(cffi:defcfun ("aiQuaternionNormalize" %assimp::quaternion-normalize)
              :void
              (%assimp::q (:pointer (:struct %assimp::quaternion))))
...
(declaim (inline %assimp::vector2add))

(cffi:defcfun ("aiVector2Add" %assimp::vector2add)
              :void
              (%assimp::dst (:pointer (:struct %assimp::vector2d)))
              (%assimp::src (:pointer (:struct %assimp::vector2d))))

(declaim (inline %assimp::vector2are-equal))

(cffi:defcfun ("aiVector2AreEqual" %assimp::vector2are-equal)
              :int
              (%assimp::a (:pointer (:struct %assimp::vector2d)))
              (%assimp::b (:pointer (:struct %assimp::vector2d))))

(declaim (inline %assimp::vector2are-equal-epsilon))

(cffi:defcfun ("aiVector2AreEqualEpsilon"
               %assimp::vector2are-equal-epsilon)
              :int
              (%assimp::a (:pointer (:struct %assimp::vector2d)))
              (%assimp::b (:pointer (:struct %assimp::vector2d)))
              (%assimp::epsilon :float))

(declaim (inline %assimp::vector2divide-by-scalar))

(cffi:defcfun ("aiVector2DivideByScalar"
               %assimp::vector2divide-by-scalar)
              :void
              (%assimp::dst (:pointer (:struct %assimp::vector2d)))
              (%assimp::s :float))

(declaim (inline %assimp::vector2divide-by-vector))

(cffi:defcfun ("aiVector2DivideByVector"
               %assimp::vector2divide-by-vector)
              :void
              (%assimp::dst (:pointer (:struct %assimp::vector2d)))
              (%assimp::v (:pointer (:struct %assimp::vector2d))))

(declaim (inline %assimp::vector2dot-product))

(cffi:defcfun ("aiVector2DotProduct" %assimp::vector2dot-product)
              :float
              (%assimp::a (:pointer (:struct %assimp::vector2d)))
              (%assimp::b (:pointer (:struct %assimp::vector2d))))

(declaim (inline %assimp::vector2length))

(cffi:defcfun ("aiVector2Length" %assimp::vector2length)
              :float
              (%assimp::v (:pointer (:struct %assimp::vector2d))))

(declaim (inline %assimp::vector2negate))

(cffi:defcfun ("aiVector2Negate" %assimp::vector2negate)
              :void
              (%assimp::dst (:pointer (:struct %assimp::vector2d))))

(declaim (inline %assimp::vector2normalize))

(cffi:defcfun ("aiVector2Normalize" %assimp::vector2normalize)
              :void
              (%assimp::v (:pointer (:struct %assimp::vector2d))))

(declaim (inline %assimp::vector2scale))

(cffi:defcfun ("aiVector2Scale" %assimp::vector2scale)
              :void
              (%assimp::dst (:pointer (:struct %assimp::vector2d)))
              (%assimp::s :float))

(declaim (inline %assimp::vector2square-length))

(cffi:defcfun ("aiVector2SquareLength" %assimp::vector2square-length)
              :float
              (%assimp::v (:pointer (:struct %assimp::vector2d))))

(declaim (inline %assimp::vector2subtract))

(cffi:defcfun ("aiVector2Subtract" %assimp::vector2subtract)
              :void
              (%assimp::dst (:pointer (:struct %assimp::vector2d)))
              (%assimp::src (:pointer (:struct %assimp::vector2d))))

(declaim (inline %assimp::vector2sym-mul))

(cffi:defcfun ("aiVector2SymMul" %assimp::vector2sym-mul)
              :void
              (%assimp::dst (:pointer (:struct %assimp::vector2d)))
              (%assimp::other (:pointer (:struct %assimp::vector2d))))

(declaim (inline %assimp::vector3add))

(cffi:defcfun ("aiVector3Add" %assimp::vector3add)
              :void
              (%assimp::dst (:pointer (:struct %assimp::vector3d)))
              (%assimp::src (:pointer (:struct %assimp::vector3d))))

(declaim (inline %assimp::vector3are-equal))

(cffi:defcfun ("aiVector3AreEqual" %assimp::vector3are-equal)
              :int
              (%assimp::a (:pointer (:struct %assimp::vector3d)))
              (%assimp::b (:pointer (:struct %assimp::vector3d))))

(declaim (inline %assimp::vector3are-equal-epsilon))

(cffi:defcfun ("aiVector3AreEqualEpsilon"
               %assimp::vector3are-equal-epsilon)
              :int
              (%assimp::a (:pointer (:struct %assimp::vector3d)))
              (%assimp::b (:pointer (:struct %assimp::vector3d)))
              (%assimp::epsilon :float))

(declaim (inline %assimp::vector3cross-product))

(cffi:defcfun ("aiVector3CrossProduct" %assimp::vector3cross-product)
              :void
              (%assimp::dst (:pointer (:struct %assimp::vector3d)))
              (%assimp::a (:pointer (:struct %assimp::vector3d)))
              (%assimp::b (:pointer (:struct %assimp::vector3d))))

(declaim (inline %assimp::vector3divide-by-scalar))

(cffi:defcfun ("aiVector3DivideByScalar"
               %assimp::vector3divide-by-scalar)
              :void
              (%assimp::dst (:pointer (:struct %assimp::vector3d)))
              (%assimp::s :float))

(declaim (inline %assimp::vector3divide-by-vector))

(cffi:defcfun ("aiVector3DivideByVector"
               %assimp::vector3divide-by-vector)
              :void
              (%assimp::dst (:pointer (:struct %assimp::vector3d)))
              (%assimp::v (:pointer (:struct %assimp::vector3d))))

(declaim (inline %assimp::vector3dot-product))

(cffi:defcfun ("aiVector3DotProduct" %assimp::vector3dot-product)
              :float
              (%assimp::a (:pointer (:struct %assimp::vector3d)))
              (%assimp::b (:pointer (:struct %assimp::vector3d))))

(declaim (inline %assimp::vector3length))

(cffi:defcfun ("aiVector3Length" %assimp::vector3length)
              :float
              (%assimp::v (:pointer (:struct %assimp::vector3d))))

(declaim (inline %assimp::vector3less-than))

(cffi:defcfun ("aiVector3LessThan" %assimp::vector3less-than)
              :int
              (%assimp::a (:pointer (:struct %assimp::vector3d)))
              (%assimp::b (:pointer (:struct %assimp::vector3d))))

(declaim (inline %assimp::vector3negate))

(cffi:defcfun ("aiVector3Negate" %assimp::vector3negate)
              :void
              (%assimp::dst (:pointer (:struct %assimp::vector3d))))

(declaim (inline %assimp::vector3normalize))

(cffi:defcfun ("aiVector3Normalize" %assimp::vector3normalize)
              :void
              (%assimp::v (:pointer (:struct %assimp::vector3d))))

(declaim (inline %assimp::vector3normalize-safe))

(cffi:defcfun ("aiVector3NormalizeSafe"
               %assimp::vector3normalize-safe)
              :void
              (%assimp::v (:pointer (:struct %assimp::vector3d))))

(declaim (inline %assimp::vector3rotate-by-quaternion))

(cffi:defcfun ("aiVector3RotateByQuaternion"
               %assimp::vector3rotate-by-quaternion)
              :void
              (%assimp::v (:pointer (:struct %assimp::vector3d)))
              (%assimp::q (:pointer (:struct %assimp::quaternion))))

(declaim (inline %assimp::vector3scale))

(cffi:defcfun ("aiVector3Scale" %assimp::vector3scale)
              :void
              (%assimp::dst (:pointer (:struct %assimp::vector3d)))
              (%assimp::s :float))

(declaim (inline %assimp::vector3square-length))

(cffi:defcfun ("aiVector3SquareLength" %assimp::vector3square-length)
              :float
              (%assimp::v (:pointer (:struct %assimp::vector3d))))

(declaim (inline %assimp::vector3subtract))

(cffi:defcfun ("aiVector3Subtract" %assimp::vector3subtract)
              :void
              (%assimp::dst (:pointer (:struct %assimp::vector3d)))
              (%assimp::src (:pointer (:struct %assimp::vector3d))))

(declaim (inline %assimp::vector3sym-mul))

(cffi:defcfun ("aiVector3SymMul" %assimp::vector3sym-mul)
              :void
              (%assimp::dst (:pointer (:struct %assimp::vector3d)))
              (%assimp::other (:pointer (:struct %assimp::vector3d))))
...
(eval-when (:load-toplevel :compile-toplevel :execute)
  ...
  (export '%assimp::+matkey-clearcoat-factor+ :%assimp)
  ...
  (export '%assimp::+config-import-schema-document-provider+
          :%assimp)
  ...
  (export '%assimp::vector3normalize-safe :%assimp)
  ...
  (export '%assimp::matrix4from-matrix3 :%assimp)
  (export '%assimp::light :%assimp)
  (export '%assimp::+quaternion-h-inc+ :%assimp)
  (export '%assimp::b :%assimp)
  (export '%assimp::+matkey-volume-attenuation-distance+ :%assimp)
  ...
  (export '%assimp::+matkey-volume-thickness-texture+ :%assimp)
  (export '%assimp::num-rotation-keys :%assimp)
  (export '%assimp::num-cameras :%assimp)
  (export '%assimp::+matkey-sheen-roughness-factor+ :%assimp)
  (export '%assimp::+assimp-h-inc+ :%assimp)
  (export '%assimp::+pushpack-is-defined+ :%assimp)
  (export '%assimp::matrix4rotation-x :%assimp)
  (export '%assimp::vector2add :%assimp)
  ...
  (export '%assimp::matrix4decompose-no-scaling :%assimp)
  (export '%assimp::duration :%assimp)
  (export '%assimp::matrix4decompose-into-scaling-axis-angle-position
          :%assimp)
  ...
  (export '%assimp::+matkey-use-color-map+ :%assimp)
  ...
  (export '%assimp::vector2length :%assimp)
  ...
  (export '%assimp::matrix4rotation-y :%assimp)
  (export '%assimp::scene :%assimp)
  (export '%assimp::+matrix3x3-inl-inc+ :%assimp)
  (export '%assimp::vector3less-than :%assimp)
  ...
  (export '%assimp::+matkey-emissive-intensity+ :%assimp)
  ...
  (export '%assimp::+matkey-roughness-factor+ :%assimp)
  ...
  (export '%assimp::+config-import-mdl-hl1-read-animations+ :%assimp)
  (export '%assimp::+material-h-inc+ :%assimp)
  (export '%assimp::+quaternion-inl-inc+ :%assimp)
  (export '%assimp::+wont-return-suffix+ :%assimp)
  (export '%assimp::+matkey-use-metallic-map+ :%assimp)
  (export '%assimp::+config-import-mdl-hl1-read-hitboxes+ :%assimp)
  ...
  (export '%assimp::matrix3are-equal :%assimp)
  (export '%assimp::ach-format-hint :%assimp)
  (export '%assimp::user-data :%assimp)
  (export '%assimp::quaternion-normalize :%assimp)
  (export '%assimp::quaternion-multiply :%assimp)
  ...
  (export '%assimp::+fp-zero+ :%assimp)
  (export '%assimp::d3 :%assimp)
  (export '%assimp::+matkey-sheen-color-factor+ :%assimp)
  (export '%assimp::release-import :%assimp)
  (export '%assimp::+config-import-fbx-read-cameras+ :%assimp)
  (export '%assimp::+config-import-mdl-hl1-read-blend-controllers+
          :%assimp)
  ...
  (export '%assimp::quaternion-interpolate :%assimp)
  (export '%assimp::+matkey-metallic-texture+ :%assimp)
  ...
  (export '%assimp::+process-preset-target-realtime-max-quality+
          :%assimp)
  (export '%assimp::+matkey-use-roughness-map+ :%assimp)
  ...
  (export '%assimp::+matkey-clearcoat-texture+ :%assimp)
  ...
  (export '%assimp::+config-import-mdl-hl1-read-bone-controllers+
          :%assimp)
  (export '%assimp::vector3are-equal :%assimp)
  (export '%assimp::+matkey-use-ao-map+ :%assimp)
  (export '%assimp::+matkey-specular-factor+ :%assimp)
  (export '%assimp::+vector2d-h-inc+ :%assimp)
  (export '%assimp::quaternion-are-equal :%assimp)
  (export '%assimp::num-uv-components :%assimp)
  (export '%assimp::file-write-proc :%assimp)
  (export '%assimp::+matkey-color-ambient+ :%assimp)
  (export '%assimp::vector3sym-mul :%assimp)
  (export '%assimp::+config-pp-og-exclude-list+ :%assimp)
  (export '%assimp::vector2negate :%assimp)
  (export '%assimp::vector3square-length :%assimp)
  ...
  (export '%assimp::vector3add :%assimp)
  (export '%assimp::+config-import-irr-anim-fps+ :%assimp)
  (export '%assimp::vector3divide-by-vector :%assimp)
  (export '%assimp::+config-import-mdl-hl1-read-attachments+
          :%assimp)
  (export '%assimp::orthographic-width :%assimp)
  (export '%assimp::d4 :%assimp)
  (export '%assimp::vector2normalize :%assimp)
  (export '%assimp::matrix4determinant :%assimp)
  ...
  (export '%assimp::matrix3from-to :%assimp)
  ...
  (export '%assimp::vector2are-equal-epsilon :%assimp)
  ...
  (export '%assimp::quaternion-conjugate :%assimp)
  (export '%assimp::d2 :%assimp)
  (export '%assimp::vector2d :%assimp)
  (export '%assimp::set-import-property-float :%assimp)
  (export '%assimp::x :%assimp)
  (export '%assimp::armature :%assimp)
  (export '%assimp::quaternion-from-euler-angles :%assimp)
  (export '%assimp::vector2dot-product :%assimp)
  (export '%assimp::vector3dot-product :%assimp)
  (export '%assimp::matrix4are-equal :%assimp)
  ...
  (export '%assimp::matrix4from-scaling-quaternion-position :%assimp)
  (export '%assimp::pc-data :%assimp)
  (export '%assimp::matrix3from-quaternion :%assimp)
  ...
  (export '%assimp::+matkey-use-emissive-map+ :%assimp)
  ...
  (export '%assimp::+config-android-jni-assimp-manager-support+
          :%assimp)
  (export '%assimp::+matkey-clearcoat-normal-texture+ :%assimp)
  (export '%assimp::+config-pp-rvc-flags+ :%assimp)
  (export '%assimp::get-import-format-count :%assimp)
  (export '%assimp::+matkey-clearcoat-roughness-factor+ :%assimp)
  ...
  (export '%assimp::vector3negate :%assimp)
  (export '%assimp::matrix4is-identity :%assimp)
  (export '%assimp::name :%assimp)
  (export '%assimp::+matkey-transmission-factor+ :%assimp)
  ...
  (export '%assimp::matrix4from-to :%assimp)
  ...
  (export '%assimp::matrix3determinant :%assimp)
  ...
  (export '%assimp::quaternion-from-normalized-quaternion :%assimp)
  ...
  (export '%assimp::matrix3inverse :%assimp)
  (export '%assimp::matrix3are-equal-epsilon :%assimp)
  (export '%assimp::set-import-property-matrix :%assimp)
  (export '%assimp::num-properties :%assimp)
  (export '%assimp::sentinel :%assimp)
  (export '%assimp::+matkey-base-color+ :%assimp)
  (export '%assimp::r :%assimp)
  (export '%assimp::face :%assimp)
  (export '%assimp::+matkey-clearcoat-roughness-texture+ :%assimp)
  (export '%assimp::+matkey-volume-attenuation-color+ :%assimp)
  ...
  (export '%assimp::matrix3from-matrix4 :%assimp)
  (export '%assimp::matrix4translation :%assimp)
  (export '%assimp::num-mesh-channels :%assimp)
  (export '%assimp::matrix4from-rotation-around-axis :%assimp)
  (export '%assimp::+matkey-shader-geo+ :%assimp)
  (export '%assimp::file-close-proc :%assimp)
  (export '%assimp::+force-inline+ :%assimp)
  (export '%assimp::+matkey-roughness-texture+ :%assimp)
  ...
  (export '%assimp::matrix4are-equal-epsilon :%assimp)
  ...
  (export '%assimp::texture-coords-names :%assimp)
  ...
  (export '%assimp::vector3length :%assimp)
  (export '%assimp::morphing-method :%assimp)
  (export '%assimp::+config-export-blob-name+ :%assimp)
  (export '%assimp::matrix4decompose-into-scaling-euler-angles-position
          :%assimp)
  (export '%assimp::vertex-id :%assimp)
  (export '%assimp::matrix3from-rotation-around-axis :%assimp)
  ...
  (export '%assimp::vector3subtract :%assimp)
  ...
  (export '%assimp::vector2divide-by-vector :%assimp)
  (export '%assimp::matrix4scaling :%assimp)
  (export '%assimp::mesh :%assimp)
  (export '%assimp::+%ai-matkey-mappingmode-u-base+ :%assimp)
  (export '%assimp::+fp-normal+ :%assimp)
  ...
  (export '%assimp::matrix4add :%assimp)
  ...
  (export '%assimp::vector2square-length :%assimp)
  ...
  (export '%assimp::vector2subtract :%assimp)
  (export '%assimp::values :%assimp)
  (export '%assimp::+matkey-glossiness-factor+ :%assimp)
  ...
  (export '%assimp::+matkey-metallic-factor+ :%assimp)
  ...
  (export '%assimp::+matkey-transmission-texture+ :%assimp)
  (export '%assimp::+false+ :%assimp)
  (export '%assimp::+slm-default-max-vertices+ :%assimp)
  (export '%assimp::+fp-nan+ :%assimp)
  ...
  (export '%assimp::+config-import-mdl-hl1-read-misc-global-info+
          :%assimp)
  ...
  (export '%assimp::vector3rotate-by-quaternion :%assimp)
  (export '%assimp::vector3normalize :%assimp)
  (export '%assimp::+matkey-volume-thickness-factor+ :%assimp)
  ...
  (export '%assimp::+matkey-sheen-roughness-texture+ :%assimp)
  ...
  (export '%assimp::+config-import-fbx-read-weights+ :%assimp)
  (export '%assimp::+matkey-sheen-color-texture+ :%assimp)
  (export '%assimp::weight :%assimp)
  (export '%assimp::tangents :%assimp)
  (export '%assimp::+config-import-md5-no-anim-autoload+ :%assimp)
  (export '%assimp::+config-import-md3-load-shaders+ :%assimp)
  ...
  (export '%assimp::matrix3translation :%assimp)
  (export '%assimp::vector2scale :%assimp)
  (export '%assimp::vector3d :%assimp)
  (export '%assimp::num-lights :%assimp)
  (export '%assimp::vector3divide-by-scalar :%assimp)
  (export '%assimp::vector3cross-product :%assimp)
  (export '%assimp::+config-import-smd-keyframe+ :%assimp)
  (export '%assimp::+failure+ :%assimp)
  (export '%assimp::+matkey-base-color-texture+ :%assimp)
  ...
  (export '%assimp::quaternion-from-axis-angle :%assimp)
  (export '%assimp::+config-import-lws-anim-end+ :%assimp)
  (export '%assimp::matrix4from-euler-angles :%assimp)
  ...
  (export '%assimp::vector3scale :%assimp)
  ...
  (export '%assimp::+config-import-mdl-hl1-read-sequence-transitions+
          :%assimp)
  ...
  (export '%assimp::matrix4inverse :%assimp))
